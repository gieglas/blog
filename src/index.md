...
<div class="row pop-medium govcy-rounded-2">
    <div class="govcy-col-6 govcy-mb-3">
        ...
        <p class="govcy-fs-5">I am an interaction designer working for the <a href="https://dsf.dmrid.gov.cy" target="_blank">Digital Services Factory</a> who loves to play with code and this is where I write about stuff.</p>
    </div>
...
<li class="nav-side govcy-br-bottom-1 govcy-br-bottom-standard govcy-py-2 govcy-mb-4"><a class="govcy-fw-bold" href="{{ pag.url  }}">{{ pag.data.title }}</a>
    ...
    {% if pag.data.image %}<a href="{{ pag.url  }}"><img style="height:180px" src="{{ (site.imagesLocation ~ pag.data.image)  }}" alt="graphic for {{ pag.data.title }}" class="img-fluid govcy-mb-2"></a>{% endif %}
